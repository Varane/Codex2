...
KEYWORD_OEM_MAP: Dict[str, List[str]] = {
    # Turbochargers
    "turbo": ["11657595351", "11657649288", "06H145702S", "06H145702L", "0375J6", "0375J7", "36002657", "53039700569", "53039700586", "4937701700"],
    "turbina": ["11657595351", "11657649288", "06H145702S", "06H145702L", "0375J6", "0375J7"],
    "turbolader": ["11657649288", "06H145702S", "06H145702L", "03L198716", "03L145702T"],
    # Injectors
    "injector": ["13537585261", "06J906036", "06H906036G", "1980L0", "1980L1", "28232242", "33800-2A000", "0445116030", "0445110328", "1465A041"],
    "einspritzdüse": ["13537585261", "06J906036", "06H906036G", "1980L0"],
    "inyector": ["13537585261", "06J906036", "06H906036G", "1980L1"],
    # Cooling fans
    "fan": ["31429982", "17427640501", "31387062", "31387064", "17427640502"],
    "cooling fan": ["31429982", "17427640501", "17427640502"],
    "ventilátor": ["31429982", "17427640501"],
    # Oil filter housing
    "oil filter housing": ["11428576524", "11428506797", "1103.TQ", "06J115403Q", "11427525335", "11427612143", "03L115389C"],
    "olio filtro": ["11428576524", "11428506797", "1103.TQ"],
    "carter olio": ["11428506797", "1103.TQ"],
    # Suspension & steering
    "control arm": ["8K0407151B", "31126771893", "31126769715", "4F0407509E", "1K0407151BC"],
    "wishbone": ["8K0407151B", "4F0407509E"],
    "air spring": ["37126790078", "37126790079", "4Z7616051A", "4Z7616052A", "2113200725"],
    "shock absorber": ["31316796314", "33526796317", "8K0513035J", "1K0512011BG", "520697"],
    # Cooling components
    "radiator": ["17117573781", "17117573782", "1300262", "8K0121251L", "1K0121251J"],
    "water pump": ["11517546994", "11517586925", "03L121011", "06H121011", "16100-39436"],
    ...
    # Intake / exhaust
    "intercooler": ["17517600533", "14411EB70A", "8K0145806B", "03L145749N", "17117791677"],
...
def load_lookup() -> Dict[str, Dict[str, Dict[str, List[str]]]]:
    path = os.path.join(os.path.dirname(__file__), "oem_lookup.json")
    try:
...
def lookup_from_table(car: str, model: str, detail: str) -> List[str]:
    candidates: List[str] = []
    if not LOOKUP_DATA:
        return candidates
    ...
    car_key = best_key(list(LOOKUP_DATA.keys()), car)
    if not car_key:
        return candidates

    model_data = LOOKUP_DATA.get(car_key, {})
    model_key = best_key(list(model_data.keys()), model) if model_data else None
    if not model_key:
        return candidates

    detail_data = model_data.get(model_key, {})
    detail_key = best_key(list(detail_data.keys()), detail) if detail_data else None
    if detail_key:
...
def extract_oems_from_text(text: str) -> List[str]:
    ...
    return [match if isinstance(match, str) else "".join(match) for match in results]
...
def resolve_oem(car: str, model: str, detail: str, query: str) -> List[str]:
    ...
    candidates.extend(lookup_from_table(car, model, detail))
    candidates.extend(keyword_oems(query))
    candidates.extend(extract_oems_from_text(query))

    normalized_query = normalize(query)
    for pattern in PREFIX_HINT_PATTERNS:
        candidates.extend(pattern.findall(normalized_query))

    if not candidates:
        keywords = [kw for kw in normalized_query.split() if kw not in STOPWORDS and len(kw) > 2]
        candidates.extend(scrape_rrr_for_keywords(keywords[:3]))

    final_candidates = dedupe([c.upper() for c in candidates])
